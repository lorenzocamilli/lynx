...
- Phase 5 — sensitive-header redaction (opt-in). Added `RedactHeaders []string` to `config.Config` and `reqlog.Config`/`Service`; when non-empty, the named headers are masked (`[REDACTED]`) before a request/response is stored. Default empty = no redaction, so captured traffic is stored verbatim (the pentest-friendly default). Redaction is applied only to copies: request headers via `clone.Header.Clone()` after scope matching (never touches the forwarded request or scope rules); response headers in-place on the already-cloned `resLog.Header`. New `pkg/reqlog/redact.go` (`redactHeaders` helper, case-insensitive, collapses multi-value headers to a single mask) with `redact_test.go` (4 sub-tests). Settings UI gained a comma-separated "Redact headers" field backed by a dedicated raw-string state (parsed to an array only on save, so the separator stays typeable). (`pkg/config/config.go`, `pkg/reqlog/reqlog.go`, `pkg/reqlog/redact.go`, `pkg/reqlog/redact_test.go`, `cmd/lynx/lynx.go`, `admin/src/features/settings/components/Settings.tsx`)
- Phase 5 — configurable log level. Added `LogLevel string` to `config.Config` (default `"info"`, new `DefaultLogLevel` const); `NewZapLogger` now takes a `levelStr` param and applies it via `zapcore.ParseLevel` over the encoding-derived default; `main.go` passes `cfg.LogLevel`. The settings POST handler defaults empty → `info` and rejects invalid levels with 400. Added a "Log level" dropdown (debug/info/warn/error) to the Settings → Application tab; the config object already round-trips all fields so the value persists. Typecheck + lint clean. Requires restart to take effect (logger built at startup). (`pkg/config/config.go`, `pkg/log/log.go`, `cmd/lynx/main.go`, `cmd/lynx/lynx.go`, `admin/src/features/settings/components/Settings.tsx`)
